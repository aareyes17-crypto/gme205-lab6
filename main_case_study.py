...
def main():
    ...
    # 1. Create parcels
    # ---------------------------------------------------------
    p1 = Parcel(parcel_id="p101", geometry={"x": 15, "y": 20}, area=650.0, zone="Residential")
    p2 = Parcel(parcel_id="p102", geometry={"x": 50, "y": 25}, area=1200.0, zone="Commercial")
    p3 = Parcel(parcel_id="p103", geometry={"x": 30, "y": 45}, area=900.0, zone="Industrial")

    # ---------------------------------------------------------
    # 2. Create buildings and assign them to parcels
    # ---------------------------------------------------------
    b1 = Building(building_id="b101", geometry={"x": 16, "y": 21}, height=15.5, usage="Residential", parcel=p1)
    b2 = Building(building_id="b102", geometry={"x": 51, "y": 26}, height=30.0, usage="Office", parcel=p2)
    b3 = Building(building_id="b103", geometry={"x": 31, "y": 46}, height=25.0, usage="Warehouse", parcel=p3)

    # ---------------------------------------------------------
    # 3. Create roads and connect them to parcels
    # ---------------------------------------------------------
    r1 = Road(road_id="r1", geometry={"x": 45, "y": 40}, length=200.0, road_type="Primary")
    r2 = Road(road_id="r2", geometry={"x": 20, "y": 30}, length=150.0, road_type="Secondary")
    ...
    # ---------------------------------------------------------
    hh1 = Household("hh1", 4, 35000.0, "Owner", building=b1)
    hh2 = Household("hh2", 2, 22000.0, "Renter", building=b2)
    hh3 = Household("hh3", 5, 45000.0, "Owner", building=b3)
    hh4 = Household("hh4", 3, 30000.0, "Renter", building=b1)
    ...
    buildings = [b1, b2, b3]
    richer_building = max(buildings, key=lambda b: b.total_household_income())
    print(f"\nBuilding with highest total household income: {richer_building.building_id} (₱{richer_building.total_household_income():.2f})")

    total_people = sum(h.num_people for h in [hh1, hh2, hh3, hh4])
    print(f"\nTotal people across all households: {total_people}")

    avg_income = sum(h.income for h in [hh1, hh2, hh3, hh4]) / 4
